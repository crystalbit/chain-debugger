...
export const transferStepHandler: StepHandler<TransferStep> = {
  ...
  process: async (step: TransferStep, stepIndex: number, context: StepContext) => {
    ...
    const command = `cast send ${step.to} --from ${step.from} --value ${step.value} --unlocked --rpc-url ${rpcUrl}`;
    ...
    try {
      ...
      if (resultData.status === 1) {
        ...
        const traceCommand = `cast call ${step.to} --from ${step.from} --value ${step.value} --rpc-url ${rpcUrl} --trace`;
        console.log(`Getting trace with command: ${traceCommand}`);
